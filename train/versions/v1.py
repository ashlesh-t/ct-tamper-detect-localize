"""
Robust Baseline Model for CT Tampering Localization - V4 (Unified Model)

Key Improvements:
- V4: Abolished the 1-channel/2-channel paths for a single, unified 2-channel model.
- The model is *always* fed a 2-channel tensor.
- Training Input: [CT, Noisy_Hint] OR [CT, Zeros]
- Validation Input: [CT, Zeros]
- This forces the model to learn the CT-only pathway and ensures
  the validation logic is identical to a portion of the training logic.
"""
import os
import json
import random
...
class Config:
    ...
    OUTPUT_DIR = "/kaggle/working/baseline_robust_v4"
    ...
    # --- Robust Training ---
    # 1. Channel Dropout
    # Probability of dropping the heatmap channel during training
    # (by replacing it with an all-zero tensor)
    CHANNEL_DROPOUT_RATE = 0.5 
    
    # 2. Hint Augmentation
    # How much to "fade" the input heatmap hint
    HINT_FADE_MIN = 0.4
    HINT_FADE_MAX = 0.9
    # How much noise to add to the input heatmap hint
    HINT_NOISE_STD = 0.15
...
class CTTamperingDataset(Dataset):
    ...
    def _augment(self, ct_img, heatmap, target_mask):
        ...
        # Add noise to the hint
        if random.random() > 0.5:
            noise = np.random.normal(0, self.config.HINT_NOISE_STD, heatmap.shape).astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if random.random() < self.config.CHANNEL_DROPOUT_RATE:
...
class ImprovedUNet(nn.Module):
    ...
    def __init__(self, in_channels=2, base_channels=64):
        ...
        # Encoder
        # *** PATCH: Removed enc1_single. This is the ONLY input path. ***
    ...
    def forward(self, x):
        # Encoder
        # *** PATCH: Removed if/else logic. Single unified path. ***
        # The input x is ALWAYS (B, 2, H, W)
        # Train: [CT, Noisy_Hint] or [CT, Zeros]
        # Val:   [CT, Zeros]
        # The model must learn to handle the zeroed channel.
        e1 = self.enc1(x)
...
def main():
    ...
    print("="*80)
    print("ROBUST CT TAMPERING LOCALIZATION - V4 (Unified Model)")
    print("="*80)
    print(f"Device: {config.DEVICE}")
    print(f"Input Channels: {config.IN_CHANNELS} (CT + Heatmap/Zeros)")
    print(f"Channel Dropout Rate: {config.CHANNEL_DROPOUT_RATE}")
    ...
    # *** PATCH: Removed weight copying, as enc1_single no longer exists ***
